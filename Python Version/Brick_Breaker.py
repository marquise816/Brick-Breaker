...
I made this following his tutorail when I was first learning python
'''

pygame.init() #Initializes things like font
FPS = 60
PADDLE_WIDTH = 100
PADDLE_HEIGHT = 15
BALL_RADIUS = 15
...
class Brick:
    ...
    def collide(self,ball):
        if not(ball.x - BALL_RADIUS < self.x + self.width and ball.x + BALL_RADIUS >= self.x):
            return False
    ...
    def interpolate(color_a, color_b, t):
        ...
        # 't' is a value betwenn 0.0 and 1.0
...
def ball_paddle_colliosion(ball, paddle):
    if not(ball.x - BALL_RADIUS < paddle.x + paddle.width and ball.x + BALL_RADIUS >= paddle.x):
        return
...
def generate_bricks(rows, cols):
    ...
    for row in range(rows):
        for col in range(cols):
            brick = Brick(col * brick_width + gap *col, row * brick_height + gap * row, brick_width, brick_height, 2, [(255,0,0), (165,42,42)])
            bricks.append(brick)
    return bricks
...
def main():
    ...
    def display_text(text):
        text_render = LIVES_FONT.render(f'You {text}!', 1, 'red')
    ...
    while run:
        ...
        ball.move()
        ball_collision(ball)
        ball_paddle_colliosion(ball, paddle)
